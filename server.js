...
const SuperBoard = require('./utilities/super-board')
...
const makeRoom = (resolve) =>{
    ...
    rooms.set(newRoom, {roomId: newRoom, players:[], superBoard:null})
    resolve(newRoom)
}
...
io.on('connection', socket =>{
    ...
    socket.on('newRoomJoin', ({room,name})=>{
        ...
        try {
            ...
            if (peopleInRoom===2){
                ...
                const gameState = currentRoom.superBoard
                const turn = currentRoom.superBoard.turn
    ...
    socket.on('move', ({room, piece, boardIndex, index}) => {
        currentSuperBoard = rooms.get(room).superBoard
        currentSuperBoard.move(boardIndex, index, piece)

        if (currentSuperBoard.checkWinner(piece)){
            io.to(room).emit('winner', {gameState:currentSuperBoard, id:socket.id})
        }else if(currentSuperBoard.checkDraw()){
            io.to(room).emit('draw', {gameState:currentSuperBoard})
        }else{
            currentSuperBoard.switchTurn()
            io.to(room).emit('update', {gameState:currentSuperBoard})
        }
    })
    ...
    socket.on('playAgainRequest', (room) => {
        ...
        io.to(room).emit('restart', {gameState:currentRoom.currentSuperBoard})
    })
